...
struct MapStyleUtility {
    ...
    static func getMaxSpan() -> MKCoordinateSpan {
        // 返回一个最大的缩放级别，避免过度缩小
        // 根据图片设置为0.9
        return MKCoordinateSpan(latitudeDelta: 0.9, longitudeDelta: 0.9)
    }
    ...
    static func ensureRegionWithinLimits(_ region: MKCoordinateRegion) -> MKCoordinateRegion {
        ...
        let latDelta = min(max(region.span.latitudeDelta, minSpan.latitudeDelta), maxSpan.latitudeDelta)
        let longDelta = min(max(region.span.longitudeDelta, minSpan.longitudeDelta), maxSpan.longitudeDelta)
    ...
    static func isHighZoomLevel(_ region: MKCoordinateRegion) -> Bool {
        return region.span.latitudeDelta < 0.15 // 修改为与图示缩放级别一致
    }
    ...
    static func isMinZoomLevel(_ region: MKCoordinateRegion) -> Bool {
        return region.span.latitudeDelta >= 0.9 // 根据图片设置最大缩放限制
    }
    ...
    static func applyCustomStyle(to mapView: MKMapView) {
        ...
        if #available(iOS 16.0, *) {
            ...
            // 注意：MKStandardMapConfiguration没有showsBuildings属性
            // config.showsBuildings = true
    ...
    static func applyMapStyleModifiers<Content: View>(_ content: Content) -> some View {
        return content
            ...
            .tint(Color.navigationRouteColor) // 使用导航路线颜色
            .onAppear {
                // 在视图出现时强制使用浅色模式
                enforceLightMode()
